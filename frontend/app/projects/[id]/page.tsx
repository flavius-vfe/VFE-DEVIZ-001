...
function SectionCard({section,trail,busy,mutate}:{section:Section;trail:string;busy:boolean;mutate:(a:()=>Promise<unknown>)=>Promise<boolean>}) {
  const [editing,setEditing]=useState(false),[adding,setAdding]=useState(false),[addingCatalog,setAddingCatalog]=useState(false),[editingItem,setEditingItem]=useState<number|null>(null);
  ...
  return <div className="hierarchy section"><Panel><div className="toolbar push"><div><div className="muted">{trail} / Capitol</div><h3>{section.code} — {section.name}</h3></div><div className="toolbar"><button className="secondary" onClick={()=>setEditing(true)}>Editează</button><button className="danger" disabled={busy} onClick={()=>{if(confirm(`Ștergi capitolul „${section.name}” și toate articolele?`))mutate(()=>api(`/api/sections/${section.id}`,{method:"DELETE"}));}}>Șterge</button></div></div><Subtotal items={section.items} label="Subtotal capitol"/><div className="toolbar"><button className="primary" onClick={()=>setAdding(true)}>Adaugă articol</button><button className="secondary" onClick={()=>setAddingCatalog(true)}>Adaugă lucrare din catalog</button></div>{addingCatalog&&<CatalogItemForm disabled={busy} onCancel={()=>setAddingCatalog(false)} onSave={async p=>{if(await mutate(()=>api(`/api/sections/${section.id}/items/from-catalog`,{method:"POST",body:JSON.stringify(p)})))setAddingCatalog(false)}}/>}{adding&&<ItemForm disabled={busy} onCancel={()=>setAdding(false)} onSave={async p=>{if(await mutate(()=>api(`/api/sections/${section.id}/items`,{method:"POST",body:JSON.stringify(p)})))setAdding(false);}}/>}{section.items.length===0&&!adding&&!addingCatalog?<div className="empty" style={{marginTop:12}}>Capitolul nu are articole de deviz.</div>:section.items.map(item=>editingItem===item.id?<ItemForm key={item.id} initial={item} disabled={busy} onCancel={()=>setEditingItem(null)} onSave={async p=>{if(await mutate(()=>api(`/api/estimate-items/${item.id}`,{method:"PUT",body:JSON.stringify(p)})))setEditingItem(null);}}/>:<div className="item-row" key={item.id}><strong>{item.code}</strong><div>{item.description}{item.notes&&<div className="muted">{item.notes}</div>}</div><span>{Number(item.quantity).toLocaleString("ro-RO")} {item.unit}</span><span>Pierderi {Number(item.waste_percent).toLocaleString("ro-RO")}%</span><strong>{(Number(item.quantity)*(1+Number(item.waste_percent)/100)).toLocaleString("ro-RO",{maximumFractionDigits:3})} {item.unit}</strong><div className="toolbar"><button className="secondary" onClick={()=>setEditingItem(item.id)}>Editează</button><button className="danger" disabled={busy} onClick={()=>{if(confirm(`Ștergi articolul „${item.description}”?`))mutate(()=>api(`/api/estimate-items/${item.id}`,{method:"DELETE"}));}}>Șterge</button></div></div>)}</Panel></div>;
}

function CatalogItemForm({disabled,onSave,onCancel}:{disabled:boolean;onSave:(p:any)=>Promise<void>;onCancel:()=>void}){const[works,setWorks]=useState<any[]>([]),[query,setQuery]=useState(""),[work,setWork]=useState(""),[code,setCode]=useState("CAT-01"),[quantity,setQuantity]=useState("1"),[waste,setWaste]=useState("0");useEffect(()=>{api("/api/catalog/work-items").then(setWorks)},[]);const filtered=works.filter(x=>x.active&&`${x.code} ${x.name}`.toLowerCase().includes(query.toLowerCase()));return <div className="card"><h3>Adaugă lucrare din catalog</h3><label>Caută lucrare<input value={query} onChange={e=>setQuery(e.target.value)}/></label><form className="form-grid" onSubmit={async e=>{e.preventDefault();await onSave({work_item_id:Number(work),code,quantity,waste_percent:waste})}}><label>Lucrare<select required value={work} onChange={e=>setWork(e.target.value)}><option value="">Selectează</option>{filtered.map(x=><option key={x.id} value={x.id}>{x.code} — {x.name} ({x.unit})</option>)}</select></label><label>Cod articol<input required value={code} onChange={e=>setCode(e.target.value)}/></label><label>Cantitate<input type="number" min="0.000001" step="0.000001" value={quantity} onChange={e=>setQuantity(e.target.value)}/></label><label>Pierderi %<input type="number" min="0" value={waste} onChange={e=>setWaste(e.target.value)}/></label><div><button className="primary" disabled={disabled}>Adaugă</button><button type="button" onClick={onCancel}>Renunță</button></div></form></div>}
